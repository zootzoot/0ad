...
void CDebuggingServer::GetStackFrame(std::stringstream& response, uint nestingLevel, uint threadDebuggerID)
{
	CScopeLock lock(m_Mutex);
	response.clear();
	std::stringstream stream;
	...
	for(itr=m_ThreadDebuggers.begin(); itr != m_ThreadDebuggers.end(); itr++)
	{
		...
		{
			(*itr)->GetStackFrame(stream, nestingLevel);
			if (stream.str() != "")
			{
				response <<  stream.str();
			}
	...
	for(itr=m_ThreadDebuggers.begin(); itr != m_ThreadDebuggers.end(); itr++)
	{
		if ( (*itr)->GetID() == threadDebuggerID || threadDebuggerID == 0)
		{
			if (DBG_CMD_NONE == (*itr)->GetNextDbgCmd() && (*itr)->GetIsInBreak())
			{
				...
				if (dbgCmd == DBG_CMD_CONTINUE && GetSettingSimultaneousThreadBreak())
					RequestThreadsToBreak(false);
			}
		}
	}
	// TODO: Error-handling
...
void CDebuggingServer::RequestThreadsToBreak(bool Enabled)
{
	CScopeLock lock(m_Mutex1);
	m_ThreadsBreakRequested = Enabled;
}

bool CDebuggingServer::GetThreadsBreakRequested()
{
	CScopeLock lock(m_Mutex1);
	return m_ThreadsBreakRequested;
}
...
{
	// TODO: this converting seems very weird...
...
void* CDebuggingServer::MgDebuggingServerCallback(mg_event event, struct mg_connection *conn, const struct mg_request_info *request_info)
{
	...
	case MG_NEW_REQUEST:
	{
		...
		else if (uri == "/GetStackFrame")
		{
			...
			GetStackFrame(stream, nestingLevel, threadDebuggerID);
		}
...
// TODO: always returns true atm. What should it return?
bool CDebuggingServer::ToggleBreakPoint(std::string filename, uint line)
{
...
{
	ENSURE(0 == SDL_SemWait(m_BreakPointsSem));
